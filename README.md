...
[lylogummy/Anima-3.8B](https://huggingface.co/lylogummy/Anima-3.8B) (52 blocks) is a community expansion of Anima-2.9B to 52 blocks (40 native + 12 newly-trained), paired with an optional Qwen3.5 4B cross-attention adapter for improved prompt adherence. This package only concerns itself with the **DiT block structure** of the 52-block checkpoint — the Qwen3.5 adapter is an entirely separate, additional component (a different checkpoint file with its own `semantic_attentions.*` keys, not matching any `net.blocks.N` pattern) and isn't touched by remapping at all. LoRAs and merges made for the native 40-block Anima-2.9B DiT remap onto the 52-block DiT exactly the same way earlier-generation LoRAs remap onto Anima-2.9B.
