...
# See LICENSE-Apache-2.0 in this directory for the full license text.
#
# MODIFICATIONS relative to upstream (required by NOTICE section 4b):
#   * _from_saved_state_dict(): a missing per-module `depth_embed` slice is now
#     zero-filled instead of raising RuntimeError. This is what makes a
#     checkpoint trained for one Anima block count usable on another: after the
#     block indices in its keys have been remapped, the host blocks that have no
#     trained counterpart (the 12 inserted per expansion step) simply receive no
#     weights. Those modules are exact no-ops, because LLLiteModuleDiT
#     zero-initialises `up.weight`/`up.bias`, and a zero depth embedding matches
#     the zero-initialised `depth_embeds` parameter.
#
# Nothing else is changed; keeping the rest byte-identical makes it easy to
# diff against a newer upstream release.
# ---------------------------------------------------------------------------
...
def _from_saved_state_dict(lllite: "ControlNetLLLiteDiT", weights_sd: dict) -> dict:
    ...
    if depth_slices:
        ...
        if missing:
            # MODIFIED (see file header): upstream raises here. Zero-filling makes
            # the uncovered host blocks no-ops instead, which is what a remapped
            # checkpoint needs.
            reference = next(iter(depth_slices.values()))
            for i in missing:
                depth_slices[i] = torch.zeros_like(reference)
            logger.info(
                "zero-filled depth_embed for %d module(s) with no trained "
                "counterpart (they contribute nothing)", len(missing)
            )
